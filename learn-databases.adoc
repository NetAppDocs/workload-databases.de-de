...
image:diagram-SAZ-database-architecture.png["Diagramm der eigenständigen Architektur mit einer einzigen Verfügbarkeitszone-Implementierung von Amazon FSX for NetApp ONTAP in einer einzigen Region"]
...
image:diagram-MAZ-database-architecture.png["Diagramm der Hochverfügbarkeits-Architektur mit zwei Knoten mit Failover-Cluster-Instanzen-Cluster in einer einzelnen Region"]
...
Allgemeine Fragen zu Workload Factory- oder Workload Factory-Anwendungen und -Diensten finden Sie unter link:get-help-databases.html["Erhalten Sie Hilfe zu Datenbanken für Workload Factory"].

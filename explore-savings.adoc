...
+ * vCPUs der Zielinstanz: Die Instanz hat 48 oder weniger virtuelle CPUs.  * Speicherzuweisung: Die Instanz verfügt über 128 GB oder weniger Speicher.

.Nutzung von Enterprise-Funktionen
Der Rechner überprüft, ob eine der folgenden Enterprise-Funktionen verwendet wird:

+ * Enterprise-Funktionen auf Datenbankebene * Online-Indexvorgänge * Ressourcenkontrolle * Peer-to-Peer- oder Oracle-Replikation * R/Python-Erweiterungen * Speicheroptimierte TempDB
...
+
.. Laden Sie das Bewertungsskript herunter.  Das Skript ist ein auf PowerShell basierendes Datenerfassungstool.  Es sammelt SQL Server-Konfigurations- und Leistungsdaten und lädt sie anschließend in Workload Factory hoch.  Der Migrationsberater bewertet die Daten und plant die Bereitstellung von FSx für ONTAP für Ihre SQL Server-Umgebung.
+

...
Die Bereitstellung von Microsoft SQL Server in einem privaten Netzwerk ohne ausgehende Konnektivität mit Terraform wird nur in der Region US-East-1 (N. Virginia) unterstützt.



== PostgreSQL-Unterstützung

PostgreSQL unterstützt weder Amazon CloudWatch noch die Implementierung in einem privaten Netzwerk.

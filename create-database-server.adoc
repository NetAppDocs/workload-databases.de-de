...
summary: Erstellen Sie einen neuen Microsoft SQL Server in Workload Factory für Datenbanken. 
---
= Erstellen Sie einen Datenbankserver in Workload Factory für Datenbanken
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
Die Erstellung eines neuen Microsoft SQL-Servers oder Datenbank-Hosts in Datenbanken erfordert eine FSX for ONTAP-Bereitstellung des Dateisystems und Ressourcen für Active Directory.

.Über diese Aufgabe
Sie benötigen die Zugangsdaten für das AWS Konto und die _Automate_ Berechtigungen.

Informieren Sie sich zunächst über die verfügbaren Speicherbereitstellungstypen für die Datenbank-Hostkonfiguration, die Active Directory-Bereitstellung, die Betriebsmodi von Workload Factory und die Anforderungen für diesen Vorgang.
...
--
Betriebsmodi für Workload Factory:: Workload Factory bietet drei Betriebsmodi, je nachdem, wie bequem Sie sich bei Workload Factory Ihre AWS Ressourcen managen lassen.
+
--
*_Basic_ Mode*: In dieser Betriebsart müssen Sie keine AWS-Kontoanmeldeinformationen in Workload Factory zuordnen. Sie können eine teilweise gefüllte YAML-Vorlage aus der Codebox kopieren oder herunterladen, um sie außerhalb von Workload Factory auszufüllen.

*_Read_ Mode*: In diesem Betriebsmodus geben Sie AWS-Kontoanmeldeinformationen mit Leseberechtigungen an, mit denen Sie das _Quick create_ oder _Advanced create_ Formular ausfüllen und dann kopieren oder herunterladen können. Sie können auch zu CloudFormation von Workload Factory mit den ausgefüllten Formulardetails umleiten. Und Sie werden in der Lage sein, den bereitgestellten Datenbankserver in Workload Factory zu verwalten.

*_Automate_ Mode*: In diesem Betriebsmodus stellen Sie AWS-Kontoanmeldeinformationen mit automatischen Berechtigungen bereit, mit denen Sie AWS-Ressourcen in Workload Factory erstellen und managen können.
...
Anmeldeinformationen und Berechtigungen:: Sie müssen link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html["Zugangsdaten für AWS Konten und Berechtigungen für den automatisierten Modus"^] einen neuen Datenbankhost in Workload Factory erstellen.
+
--
Alternativ können Sie mit der Codebox eine leere Vorlage kopieren oder eine fertige Vorlage erstellen, sodass Sie mit REST-API, AWS CLI oder AWS CloudFormation eine Datenbank außerhalb von Workload Factory bereitstellen können. link:https://docs.netapp.com/us-en/workload-setup-admin/codebox-automation.html["Erfahren Sie mehr über die Codebox-Automatisierung"^].
...
Sie können die Bereitstellungsmodi _Quick create_ oder _Advanced create_ verwenden, um diese Aufgabe in Workload Factory mit _Automate_-Modus-Berechtigungen abzuschließen.
...
. Wählen Sie in der Kachel Datenbanken *Deploy Database Host* aus.
...
+
Mit den AWS-Zugangsdaten mit _Automate_-Berechtigungen kann Workload Factory den neuen Datenbank-Host über Ihr AWS-Konto in Workload Factory implementieren und managen.

+
Mit AWS Zugangsdaten mit _read_ Berechtigungen kann Workload Factory eine CloudFormation-Vorlage erstellen, die Sie in der AWS CloudFormation-Konsole verwenden können.

+
Wenn Sie keine AWS-Anmeldeinformationen in Workload Factory haben und den neuen Server in Workload Factory erstellen möchten, folgen Sie *Option 1*, um zur Seite Anmeldedaten zu gelangen. Fügen Sie die erforderlichen Anmeldeinformationen und Berechtigungen für den _Automate_-Modus für Datenbank-Workloads manuell hinzu.

+
Wenn Sie das Formular zum Erstellen eines neuen Servers in Workload Factory ausfüllen möchten, damit Sie eine vollständige YAML-Dateivorlage für die Bereitstellung in AWS CloudFormation herunterladen können, folgen Sie *Option 2*, um sicherzustellen, dass Sie über die erforderlichen Berechtigungen zum Erstellen des neuen Servers in AWS CloudFormation verfügen. Fügen Sie die erforderlichen Anmeldeinformationen und Berechtigungen für den _read_-Modus für Datenbank-Workloads manuell hinzu.

+
Optional können Sie eine teilweise ausgefüllte YAML-Dateivorlage aus der Codebox herunterladen, um den Stack außerhalb von Workload Factory ohne Anmeldeinformationen oder Berechtigungen zu erstellen. Wählen Sie *CloudFormation* aus der Dropdown-Liste in der Codebox aus, um die YAML-Datei herunterzuladen.
...
+
Für ein benutzerverwaltetes Active Directory können Sie eine Sicherheitsgruppe auswählen, die Datenverkehr von Ihren vorhandenen Systemen zulässt, die eine Verbindung zur Datenbank herstellen müssen. Die Sicherheitsgruppe sollte die Kommunikation mit den Active Directory-Domänencontrollern aus den Subnetzen ermöglichen, in denen EC2-Instanzen für Microsoft SQL konfiguriert sind.
...
Nachdem der Server bereitgestellt wurde, aktiviert Workload Factory keine Remote-Verbindung auf dem Microsoft SQL Server. Führen Sie die folgenden Schritte aus, um die Remote-Verbindung zu aktivieren.
...
.Wie es weiter geht
Jetzt können Sie link:create-database.html["Erstellen Sie eine Datenbank in Workload Factory"].
